...
import { deleteClientsAPI, getClientById, listClientsAPI } from '../../APIServices/mastersAPI/clientsAPI';
  ...
  return (
    <section className='py-8 bg-gray-300 rounded-3xl'>
      <div className='container px-4 mx-auto'>
        <div className='flex flex-col space-y-4'>
          <div className='flex flex-col sm:flex-row sm:justify-between items-center pb-4 border-b bg-white rounded-lg'>
            ...
            <button className="bg-[#58ac3b] p-2 rounded-full mt-2  mr-4 sm:mt-0">Add Client</button>
          </Link>
            ...
            <div className='pb-4'>
              <div className='overflow-x-auto'>
                <table className='table-auto w-full'>
                  <thead>
                    <tr>
                      <th className='pb-3 text-black font-medium text-left p-3'>Client Name</th>
                  ...
                  <tbody>
                    {listClient.length ? (
                      listClient.map((client, index) => (
                        ...
                        >
                          <td className='py-2 px-4 text-black'>{client?.name}</td>
                          <td className='py-2 px-4 text-black'>{maskContactNumber(client?.contact)}</td>
                          <td className='py-2 px-4 text-black'>{client?.address}</td>
                          <td className='py-2 px-4'>
                            <div className='flex gap-2'>
                              <FaEye onClick={() => viewClientHandler(client._id)} className='text-blue-500 cursor-pointer' />
                              <FaPencilAlt onClick={() => handleEditClick(client._id)} className='text-yellow-500 cursor-pointer' />
                              <FaRegTrashAlt onClick={() => deleteHandler(client._id)} className='text-red-500 cursor-pointer' />
                            </div>
                          </td>
                        </tr>
                      ))
      ...
      {isModalOpen && selectedClient && (
        <ClientModal isOpen={isModalOpen} client={selectedClient} onClose={() => setModalOpen(false)} />
      )}
    </section>
  );
};
